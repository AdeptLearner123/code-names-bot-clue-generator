...
def match_paths(graph, source, rules, target, cutoff):
...
def match_paths_helper(graph, current_path, rules, target = None, cutoff = None):
    ...
    if len(rules) == 0:
        if target is None or curr_node == target:
            print("Appending path", target, curr_node, rules)
            return [current_path]
        print("Terminating because rules is empty")
        return []

    if cutoff is not None and len(current_path) == cutoff:
        print("terminating because reached cutoff")
        return []
